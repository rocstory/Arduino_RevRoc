...
void RevRoc::signalGameOver()
{
	...
	for (int i = 0; i < 20; i++)
	{
		...
		if (i % 2 == 0)
		{
			rgb.setColor(player.pixelNum, rgbBGColor[0], rgbBGColor[1], rgbBGColor[2]); // set to background
			rgb.setColor(player.pixelNum, plyrColor[0], plyrColor[1], plyrColor[2]);	// set to player color
			delay(100);
		}
		else
		{
			rgb.setColor(target.pixelNum, rgbBGColor[0], rgbBGColor[1], rgbBGColor[2]); 			// set to background
